"""These modules contain classes to represent Notebook and Notes"""
import datetime
last_id = 0
...
class Note:
    ...
    def match(self, filtering):
        ...
        return filtering in self.memo or filtering in self.tags
...
class Notebook:
    ...
    def __init__(self):
        """Initialize a notebook with an empty self.notes = []"""
        self.notes = []

    def new_note(self, memo, tags=''):
        """Create a new note and add it to the self.notes.append(Note(memo, tags))
        list."""
    ...
    def search(self, filtering):
        """Find all notes that match the given filter string."""
        return [note for note in self.notes if
                note.match(filtering)]
